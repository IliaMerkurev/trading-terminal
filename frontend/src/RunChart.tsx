...
export default function RunChart({runId,focus}:{runId:string;focus:number|null}){
  ...
  useEffect(()=>{if(focus!==null)setStart(Math.floor(focus/60)*60-60*30);},[focus]);
  ...
  const available:string[]=data?[...new Set<string>(data.series.indicators.flatMap((p:any)=>Object.keys(p.values)))]:[];
  useEffect(()=>{
    if(!container.current||!data)return;
    const chart=createChart(container.current,{autoSize:true,height:430,layout:{background:{type:ColorType.Solid,color:'#111720'},textColor:'#a5b3c9',attributionLogo:true},grid:{vertLines:{color:'#1c2532'},horzLines:{color:'#1c2532'}},timeScale:{timeVisible:true,secondsVisible:false},rightPriceScale:{borderColor:'#2b374b'}});
