...
import {createNode,graphProblems,isTextEditing} from './editor';
...
export default function App(){
  ...
  useEffect(()=>{const shortcut=(e:KeyboardEvent)=>{if(tab!=='Strategy'||kind!=='graph'||isTextEditing(e.target)||!(e.ctrlKey||e.metaKey))return;if(e.key.toLowerCase()==='z'){e.preventDefault();e.shiftKey?editor.redo():editor.undo();}else if(e.key.toLowerCase()==='y'){e.preventDefault();editor.redo();}};document.addEventListener('keydown',shortcut);return()=>document.removeEventListener('keydown',shortcut);});
  ...
  async function openStrategy(id:string){await action(async()=>{
    ...
    if(s.kind==='graph'){editor.reset(s.document.graph,s.document.layout??{});}else{setNative(s.document);setNativeDependencies(JSON.stringify(s.document.dependencies,null,2));setNativeConfig(JSON.stringify(s.document.config,null,2));setPreview(null);setTrust(false);setProfile(p=>({...p,evaluation:'closed',stop_loss:'0',take_profit:'0',version:1}));}
    setTab('Strategy');
  });}
  function newStrategy(format:'graph'|'native'){
    ...
    if(format==='graph'){editor.reset(exampleGraph(),{});}else{setNative(nativeExample());setNativeDependencies(JSON.stringify(nativeExample().dependencies,null,2));setNativeConfig(JSON.stringify(nativeExample().config,null,2));setProfile(p=>({...p,evaluation:'closed',stop_loss:'0',take_profit:'0',version:1}));}
    setTab('Strategy');
  }
  ...
  return <div className="app">
    ...
    <div className="workspace">
      ...
      <main><div className="toolbar"><button className="icon-button" title="Toggle strategy list" onClick={()=>setLeft(!left)}>☷</button><input className="strategy-title" aria-label="Strategy name" value={name} onChange={e=>setName(e.target.value)}/><span className="format-tag">{kind==='graph'?'VISUAL':'PYTHON'}</span><button disabled={busy||!isDesktop()} onClick={save}>Save</button><button onClick={copyStrategy}>Copy strategy</button><button className="icon-button" title="Toggle settings" onClick={()=>setRight(!right)}>⚙</button></div>
        <nav>{['Strategy','Backtest','Experiments','Results'].map(t=><button key={t} className={tab===t?'active':''} onClick={()=>setTab(t)}>{t}</button>)}<span>{activeId?'● RUNNING':'RESEARCH V2'}</span></nav>
