...
export default function App(){
  ...
  async function openStrategy(id:string){await action(async()=>{
    ...
    if(s.kind==='graph'){setGraph(s.document.graph);setLayout(s.document.layout??{});}else{setNative(s.document);setNativeDependencies(JSON.stringify(s.document.dependencies,null,2));setNativeConfig(JSON.stringify(s.document.config,null,2));setPreview(null);setTrust(false);setProfile(p=>({...p,evaluation:'closed',stop_loss:'0',take_profit:'0'}));}
    setTab('Strategy');
  });}
  function newStrategy(format:'graph'|'native'){
    ...
    if(format==='graph'){setGraph(exampleGraph());setLayout({});}else{setNative(nativeExample());setNativeDependencies(JSON.stringify(nativeExample().dependencies,null,2));setNativeConfig(JSON.stringify(nativeExample().config,null,2));setProfile(p=>({...p,evaluation:'closed',stop_loss:'0',take_profit:'0'}));}
    setTab('Strategy');
  }
