...
export default function LibraryResearch({selections,datasets,profile,onActive,onOpenRun,onReport}:{selections:any[];datasets:any[];profile:Profile;onActive:(id:string|null)=>void;onOpenRun:(id:string)=>void;onReport:(report:any,stale:boolean)=>void}){
 ...
 <label>Sort within matching cohorts<select disabled={holdout} aria-label="Library sort" value={sort} onChange={e=>setSort(e.target.value)}>{[['net_pnl','Net PnL'],['max_drawdown','Lowest drawdown'],['completed_positions','Completed positions'],['win_rate','Win rate']].map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>
