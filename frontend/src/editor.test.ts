...
describe('graph editing contracts',()=>{
 ...
 it('searches full names and abbreviations without fixed outputs',()=>{expect(searchNodes(' exponential moving ')).toEqual(['ema']);expect(searchNodes('rSi')).toEqual(['rsi']);expect(searchNodes('Entry Long')).toEqual([]);expect(searchNodes('')).toHaveLength(14);});
