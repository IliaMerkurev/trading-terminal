...
import {exampleGraph,catalog,canConnect} from './model';
import {GraphHistory} from './useGraphHistory';
describe('graph editing contracts',()=>{
 it('copies a numeric multiplication node and rejects Boolean inputs',()=>{const g=exampleGraph();const n=createNode('multiply');g.nodes.push(n);expect(searchNodes('multiply')).toEqual(['multiply']);expect(canConnect(g,'close','value',n.id,'left')).toBe(true);expect(canConnect(g,'above','value',n.id,'left')).toBe(false);const copied=duplicateNodes(g,{},[n.id]);expect(copied.graph.nodes.at(-1)?.type).toBe('multiply');});
 ...
 it('searches full names and abbreviations without fixed outputs',()=>{expect(searchNodes(' exponential moving ')).toEqual(['ema']);expect(searchNodes('rate of change')).toEqual(['roc']);expect(createNode('roc').params.period).toBe(1);expect(searchNodes('rSi')).toEqual(['rsi']);expect(searchNodes('Entry Long')).toEqual([]);expect(searchNodes('')).toHaveLength(21);});
