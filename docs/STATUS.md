...
There is no runnable desktop application, user-facing trusted import workflow, chart UI, comparison/export workflow, or installer. Sixty-five Windows regression tests have passed; these are calculation/data/service foundations, not V1 acceptance. Historical Bybit tier changes remain unknown. One real BTCUSDT day was downloaded for spot and perpetuals; both cached datasets produced identical repeated offline results. A seven-day synthetic intrabar fixture measured 1.464s and 260.5 MiB peak working set on Windows.

## Next milestone

Implement explicitly trusted native import (ILI-10), then the desktop editor/workflow (ILI-11 and ILI-12). [PROJECT.md](../PROJECT.md) remains the authoritative scope.
