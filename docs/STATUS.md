...
Product 0.3 is implemented on `codex/03`; main remains the accepted 0.2 baseline until a separately authorized merge. It is an early development build, not stable or production-ready. See the [0.3 demonstration](DEMO_03.md) and [scope](../PROJECT.md#19-product-03-live-monitoring-and-paper-trading).
...
### Remaining owner/native acceptance

Confirm visible toast banners, audible sound and Telegram Test using credentials entered only through the application. Check Ctrl+Z/Ctrl+Y with the owner's physical keyboard: component physical-key/capture tests pass, but the current native input helper did not demonstrate these chords (localized key injection was unavailable); Undo/Redo buttons were verified. A controlled 100%/125%/150% Windows display-scale matrix and physical sleep/wake remain manual; stale/gap reconstruction is covered deterministically and by the isolated public-connection interruption. No system setting was changed to manufacture a pass.
...
## Accepted main baseline: 0.2 development build
