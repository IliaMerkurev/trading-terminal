...
class JobTests(unittest.TestCase):
    ...
    def test_snapshot_worker_persistence_paging_and_repeated_results(self):
        ...
        chart=self.manager.store.chart_window(run_id,60,30)
        self.assertEqual(chart['start'],60)
        self.assertEqual(len(chart['series']['candles']),30)
        self.assertEqual(chart['series']['candles'][0]['time'],60)
        self.assertTrue(all(60<p['time']<=1860 for p in chart['series']['indicators']))
        with self.assertRaises(ValueError): self.manager.store.chart_window(run_id,1,10000)
